...
namespace couchdb
{
    ...
    public:
        ...
        virtual std::vector<std::string> list_db_names()
        {
            ...
            {
                if (item.asString().find('_') != 0 && item.asString().find("shards/") != 0)
                    dbs.push_back(item.asString());
            }
        ...
        virtual std::vector<database_type> list_dbs()
        {
            ...
            {
                if (item.asString().find('_') != 0 && item.asString().find("shards/") != 0)
                    dbs.push_back(database_type(comm, item.asString()));
            }
        ...
        virtual database_type create_db(const std::string &db)
        {
            ...
            if (response.isMember("error"))
            {
                ...
                std::cout << "Unable to create database \"" + db + "\": " + response["reason"].asString();
#endif
                throw error(error::database_not_creatable, response["reason"].asString());
            }
        ...
        virtual connection &remove_db(const std::string &db)
        {
            ...
            {
#ifdef CPPCOUCH_DEBUG
                std::cout << "Unable to delete database \"" + db + "\": " + response["reason"].asString();
#endif
        ...
        virtual Json::Value create_user(const std::string &name, const std::string &pass, const Json::Value &roles = Json::Value(Json::arrayValue) /* Array */)
        {
            Json::Value obj;
        ...
        virtual std::vector<std::string> list_user_names()
        {
            ...
            if (!response.isObject() || !response["rows"].isArray())
                throw error(error::bad_response);
            ...
            for (auto row: response)
            {
                ...
                std::string name = row["id"].asString();
        ...
        // Tries to create a new CouchDB user
        virtual Json::Value create_user(const user &_user, const Json::Value &roles = Json::Value(Json::arrayValue))
        {
    ...
    protected:
        virtual void get_couchdb_info()
        {
            ...
            couchDBVersion = response["version"].asString();
